...
export const communicationWithNative = (iosObj, androidObj, webObj) => {
  if (browser.versions.wechat) {
    top.postMessage(webObj, '*');
  } else if (browser.versions.android) {
    let _android;
    let method = androidObj.method,
      sendData = androidObj.args;
    javascript: (_android = android)[method].apply(_android, _toConsumableArray(sendData));
  } else {
...
// 监听手机返回键函数
export const backToTarget = (routerName = "", router = {}) => {
  document.addEventListener("plusready", function () {
    plus.key.addEventListener("backbutton", function (evt) {
    }, false);
  });
};
export const addEventBack = () => {
  // console.log("执行函数");
